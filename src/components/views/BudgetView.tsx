...
const BudgetView: React.FC = () => {
  ...
  }, [newTransaction.amount, newTransaction.currency, currency, formatMoney, t]);
  ...
  }, [transactions, sumByType, currency]); // currency dependency fontos az újraszámoláshoz
  ...
  const getTransactionAmountsByCurrency = (type: 'income' | 'expense') => {
    ...
    transactions
      .filter(tr => tr.type === type)
      .filter(tr => (tr as any).kind !== 'master')
      .forEach(tr => {
  ...
  const categoryData = useMemo(() => {
    ...
    transactions
      .filter(tr => tr.type === 'expense' && (tr as any).kind !== 'master')
      .forEach(tr => {
        const amount = Math.abs(tr.amount);
        const trCurrency = (tr as any).currency || 'USD';
  ...
  const cashFlowData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      transactions.forEach(tr => {
        if ((tr as any).kind === 'master') return;
    ...
    if (!hasData) return monthsData; // Üres adatokkal visszatérés, hogy a tengelyek látszódjanak
    return monthsData;
  ...
  const periodCounts = useMemo(() => {
    const visibleTx = transactions.filter(tr => (tr as any).kind !== 'master');
    return {
