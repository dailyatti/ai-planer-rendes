...
const InvoicingView: React.FC = () => {
    ...
    // PhD-Level Handler: Schedule tasks for ALL pending invoices
    const handleScheduleAllPendingTasks = () => {
        const pendingInvoices = invoices.filter(inv => inv.status === 'sent');
        if (pendingInvoices.length === 0) {
            showToast(language === 'hu' ? 'Nincs függő számla.' : 'No pending invoices.');
            return;
        }
        pendingInvoices.forEach(invoice => handleCreateTaskFromInvoice(invoice));
        showToast(t('invoicing.pendingTasksCreated'));
    };
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {[
                            ...
                            >
                                ...
                                {/* Schedule All Pending Action (only for pending) */}
                                {stat.id === 'pending' && invoices.filter(i => i.status === 'sent').length > 0 && (
                                    <div
                                        className="mt-3 pt-2 border-t border-gray-100 dark:border-gray-700 text-xs text-indigo-600 hover:text-indigo-800 cursor-pointer flex items-center gap-1 transition-colors"
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            handleScheduleAllPendingTasks();
                                        }}
                                    >
                                        <CalendarPlus size={12} />
                                        {t('invoicing.scheduleAllPending')}
                                    </div>
                                )}
                            </button>
                        ))}
                                        ...
                                        <div className="flex items-center gap-2">
                                            {invoice.status === 'sent' && (
                                                <button
                                                    ...
                                                    className="p-2 text-indigo-500 hover:text-white hover:bg-indigo-500 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                                                    title={t('invoicing.createTaskForDue')}
                                                >
